library(haven)
hdata <- read_dta('household_characteristics.dta')

library(tidyverse)
hdata.1 <- hdata %>% filter(village == 28)
A <- as.matrix(read.csv('adj_borrowmoney_HH_vilno_28.csv', header = F))
label <- as.matrix(read.csv('key_HH_vilno_28.csv', header= F))

# Largest connecting component
library(igraph)
colnames(A) <- rownames(A) <- label
g <- graph_from_adjacency_matrix(A)
components <- igraph::clusters(g, mode="weak")
biggest_cluster_id <- which.max(components$csize)
vert_ids <- V(g)[components$membership == biggest_cluster_id]
newg <- igraph::induced_subgraph(g, vert_ids)
newA <- as.matrix(as_adjacency_matrix(newg))
D <- distances(newg)

hdata.2 <- hdata.1 %>%
  mutate(castesubcaste=case_when(
    castesubcaste=="OBC" ~ 1,
    castesubcaste=="SCHEDULE CASTE" ~ 2,
    castesubcaste=="SCHEDULE TRIBE" ~ 3,
    castesubcaste=="MINORITY" ~ 4
  ))
...
main_bhmds <- function(n, d, D, V0, X0, sigg0, a, alpha, maxiter, constant, betas, burnin, kappa) {
  ...
  trace_h <- matrix(rep(0, (maxiter - burnin) * n), ncol = (maxiter - burnin))
  ...
  result <- c(I(list(tracedelta)), I(list(X)), I(list(trace_12)), I(list(trace_sigma)), I(list(trace_h)))
  return (result)
}

hydra <- hydraPlus(D, dim = 2, curvature = kappa, control = list(return.lorentz = T), maxit = 100000)
# hydra <- readRDS("hydra.RData")
nancount = 0
...
h <- acosh(matrix(unlist(result[5]), ncol = 80000))
med <- rep(0, n)
for (i in 1:n) {
  med[i] = median(h[i,])
}
h_ordered <- t(h)
h_ordered <- h_ordered[,order(med)]
boxplot(h_ordered[,1:15], names = colnames(D)[order(med)[1:15]])
